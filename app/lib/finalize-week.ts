...
export async function finalizeWeek(
...
): Promise<{ week: number; finalized: number; skipped: number }> {
  ...
  return { week, finalized, skipped };
}

/**
 * Finalize every still-scheduled pool matchup from an already-completed NFL
 * week. Sleeper's `state.week` points at the upcoming/current week once the
 * prior one is over, so any Fantasy Fantasy week strictly before it is
 * eligible to resolve.
 */
export async function finalizeCompletedWeeks(): Promise<{ finalizedWeeks: number[]; finalizedMatchups: number; skippedMatchups: number }> {
  ...
  for (const row of weeksResult.rows) {
    const result = await finalizeWeek(Number(row.week), client);
  ...
  return { finalizedWeeks, finalizedMatchups, skippedMatchups };
}
